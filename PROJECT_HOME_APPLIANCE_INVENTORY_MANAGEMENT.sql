...
-- Find all the manufacturer and category names for all products present
...
-- Get the name of the customer, city and the product bought. Also get the sale date and name and city of the employee who handled the order. 
...
-- Get all manufacturers name, supplier name and phone number and display average stock quantity for existing stock.
...
-- This subquery will return the average repair cost for each product that was repaired in the New York service center in the year 2022 and whose repair status is "Completed", but only for those products whose average repair cost is greater than the overall average repair cost across all completed repairs.--
...
-- Query 2:Select all the stock in price is greater than average cost price

SELECT s.StockID, s.Quantity, s.CostPrice, s.RetailPrice, s.ProductID
FROM Stock s
WHERE costprice > (SELECT AVG(Costprice) FROM stock);

-- Query3:Write a query to display all the products in ascending which are having same price
...
-- Query4:Write a query to display all the manufacturer name having same city location 
...
-- 1.Query that joins the Supplier and Stock tables, and uses the SUM function to calculate the total quantity of each product supplied by each supplier:
...
/* 2. Supplier and Stock tables and retrieves data for all products that have a retail price higher than $50, ordered by the quantity in stock in descending order: */
...
-- 3.Query that joins the Supplier and Stock tables and uses a subquery to retrieve the product with the highest retail price for each supplier:
...
-- Queriesformaintenance1.Showthetotalmaintenancecostforeachproduct:
SELECT productid,SUM(cost)AStotal_cost FROM maintenance GROUP BY productid;

-- 2.Showthenumberofmaintenanceandrepairtasksperformedforeachproduct:
...
-- Stocktablequeries1.Toretrievethetotalquantityandcostpriceofallproductsinstock:
SELECT SUM(quantity) AS total_quantity,SUM(costprice*quantity) AS total_cost FROM stock;

--  2.Toretrievetheproductwiththehighestretailprice:
SELECT * FROM stock WHERE retailprice = (SELECT MAX(retailprice) FROM stock);

-- 3.Toretrievetheproductinformationalongwithitssupplierandmanufacturerinformation:
...
-- DISCOUNTTABLEQUERIES1.Retrievealldiscountsforaspecificproduct:
SELECT * FROM discount WHERE productid = 3001;

-- 2.Retrievealldiscountsthatarecurrentlyvalid:
SELECT * FROM discount WHERE startdate <= CURDATE() AND enddate >= CURDATE();

-- 3.Retrievethetotaldiscountamountforallproducts:
